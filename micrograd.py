...
# What does the value represent in the neural network?
...
class Value:
    ...
    def __init__(self, data, _prev=(), _op='', type=ValueType.Input, layer_idx=None):
        self.data = data
        self.grad = 0
        # internal variables used for autograd graph construction
        self._backward = lambda: None
        self._prev = _prev
        # for visualization / debugging / etc
        self._op = _op # the op that produced this node (e.g., '+', '*', 'tanh', etc)
        self.type = type
        self.layer_idx = layer_idx 

    def __add__(self, other):
        other = other if isinstance(other, Value) else Value(other)
        out = Value(self.data + other.data, (self, other), '+', type=ValueType.Intermediate, layer_idx=self.layer_idx)
    ...
    def __pow__(self, other):
        ...
        out = Value(self.data**other, (self,), f'**{other}', type=ValueType.Intermediate, layer_idx=self.layer_idx)
    ...
    def relu(self):
        out = Value(0 if self.data < 0 else self.data, (self,), 'ReLU', type=ValueType.Intermediate, layer_idx=self.layer_idx)
    ...
    def tanh(self):
        out = Value(math.tanh(self.data), (self,), 'tanh', type=ValueType.Intermediate, layer_idx=self.layer_idx)
    ...
    def exp(self):
        out = Value(math.exp(self.data), (self,), 'exp', type=ValueType.Intermediate, layer_idx=self.layer_idx)
    ...
    def log(self):
        ...
        out = Value(math.log(self.data), (self,), 'log', type=ValueType.Intermediate, layer_idx=self.layer_idx)
    ...
    def __repr__(self):
        return f"Value(data={self.data}, grad={self.grad}, type={self.type})"
...
class Neuron(Module):
    ...
    def __init__(self, nin, nonlin=True, layer_idx=None):
        self.w = [Value(random.uniform(-1, 1) * nin**-0.5, type=ValueType.Weight, layer_idx=layer_idx) for _ in range(nin)]
        self.b = Value(0, type=ValueType.Bias, layer_idx=layer_idx)
        self.nonlin = nonlin
        self.layer_idx = layer_idx
...
class MLP(Module):
    ...
    def __init__(self, nin, nouts):
        sz = [nin] + nouts
        self.layers = [Layer(sz[i], sz[i+1], nonlin=i!=len(nouts)-1, layer_idx=i+1) for i in range(len(nouts))]
...
if __name__ == "__main__":
    ...
    def loss_fun(model, split):
        ...
        total_loss = Value(0.0, type=ValueType.Loss)
        for x, y in split:
            logits = model(x)
    ...
    x, y = (Value(0.0, layer_idx=0), Value(0.0, layer_idx=0)), 0
